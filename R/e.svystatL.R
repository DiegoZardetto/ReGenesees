svystatL <- function(design, expr, by = NULL,
...
if (!is.null(by)) {
    ...
    stat <- svyby(formula = expr, by = by, design = design, FUN = svylin,
    ...
    class(stat) <- c("svystatL.by", class(stat))
    attr(stat,"design") <- design.expr
    return(stat)
    }
else {
    stat <- svyby(formula = expr, by = rep(1, nrow(design)), design = design, FUN = svylin,
                  deff = deff, keep.var = TRUE, keep.names = TRUE,
                  verbose = FALSE, vartype = vartype, ci.lev = conf.lev,
                  drop.empty.groups = TRUE, covmat = FALSE, na.rm = na.rm,
                  return.replicates = FALSE)
  ...
  # Now change stat's format (recall stat is a dataframe)    
    ...
    colnames(stat.mat) <- c("Complex", varia.name, if (attr(stat, "svyby")$deffs) "DEff")
    # Cast into a data.frame
    stat.df <- as.data.frame(stat.mat)
    attr(stat.df, "origin") <- stat
    class(stat.df) <- c("svystatL", class(stat.df))
    # Better column names
...
SE.svystatL <- function(object, ...) SE(attr(object, "origin"))

VAR.svystatL <- function(object, ...) VAR(attr(object, "origin"))

cv.svystatL <- function(object, ...) cv(attr(object, "origin"))

deff.svystatL <- function(object, ...) deff(attr(object, "origin"))

confint.svystatL <- function(object, ...){
